...
(defun prime-sieve (limit)
  ...
  (setq *rawbits* (make-array (ceiling limit 2)))

  (dotimes (i (length *rawbits*))
    (setf (svref *rawbits* i) 1)))


(defmacro get-bit (rawbits index)
  `(svref ,rawbits (floor ,index 2)))


(defmacro clear-bit (rawbits index)
  `(setf (svref ,rawbits (floor ,index 2)) 0))
...
(let* ((duration (/ (- (get-internal-real-time) *start*) internal-time-units-per-second))
  ...
  (format t "array;~d;~f;1;algorithm=base,faithful=no,bits=unknown~%" *passes* duration))
