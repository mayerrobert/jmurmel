...
(defun m%print-integer (arguments output-stream base colonp atp params)
  (if (integerp (car arguments))
      (let* ((arg (car arguments))
             (mincol (car params))
             (params (cdr params))
             (padchar (char-with-default (car params) #\ ))
             (params (cdr params))
        ...
        (if colonp
            ...
            (let* ((commachar (char-with-default (car params) #\,))
                   (comma-interval (int-with-default (cadr params) 3)))
              (labels ((loop (n pos)
                        (when (< n 0)
...
(defun m%print-obj (arguments output-stream escapep colonp atp params)
  (let* ((mincol (int-with-default (car params) 0))
         (params (cdr params))
         (colinc (int-with-default (car params) 1))
         (params (cdr params))
         (minpad (int-with-default (car params) 0))
         (params (cdr params))
         (padchar (char-with-default (car params) #\ ))
         (arg (car arguments))
         (str (if arg
