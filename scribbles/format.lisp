...
(defun m%print-integer (arg output-stream base colonp atp params)
  (if (integerp arg)
...
(defun m%print-simple-integer (arg output-stream base)
  (if (integerp arg)
...
;; semi-private: used by the function generated by 'format-function' and the expansion of 'formatter'
(defun m%print-obj (arg output-stream escapep atp params)
...
(defmacro formatter (control-string)
  (let* ((body (cons () ()))
    ...
    (labels ((collect (form)
             ...
             (collect-shift (form)
               (setq append-to (cdr (rplacd append-to (cons `(progn ,form (setq arguments (cdr arguments))) nil)))))
             ...
             (do-integer (base colonp atp params)
               (collect-shift (if (or colonp atp params)
                                  `(m%print-integer        (car arguments) output-stream ,base ,colonp ,atp ',params)
                                  `(m%print-simple-integer (car arguments) output-stream ,base))))
             ...
             (if (stringp elem)
                 ...
                 (let ((colonp (cadr elem))
                   ...
                   (case (car elem)
                     ...
                     ((#\d #\D)
                      (collect-shift (if (or colonp atp params)
                                         `(m%print-integer (car arguments) output-stream 10 ,colonp ,atp ',params)
                                         `(write (car arguments) nil output-stream))))
                     ...
                     ((#\a #\A)
                      (collect-shift (if params
                                         `(m%print-obj (car arguments) output-stream nil ,atp ',params)
                                         `(write (car arguments) nil output-stream))))

                     ;; Tilde S: Standard
                     ((#\s #\S)
                      (collect-shift (if params
                                         `(m%print-obj (car arguments) output-stream t ,atp ',params)
          ...
          (let ((colonp (cadr elem))
            ...
            (labels ((do-float (c arg)
              ...
              (case (car elem)
                ...
                ;; ~nR prints arg in radix n. sbcl supports 2..36
                ((#\r #\R)
                 (m%print-integer (car arguments) output-stream (car params) colonp atp (cdr params))
                 (setq arguments (cdr arguments)))
                ...
                ;; Tilde B: Binary
                ((#\b #\B)
                 (m%print-integer (car arguments) output-stream 2 colonp atp params)
                 (setq arguments (cdr arguments)))

                ;; Tilde O: Octal
                ((#\o #\O)
                 (m%print-integer (car arguments) output-stream 8 colonp atp params)
                 (setq arguments (cdr arguments)))

                ;; Tilde X: Hexadecimal
                ((#\x #\X)
                 (m%print-integer (car arguments) output-stream 16 colonp atp params)
                 (setq arguments (cdr arguments)))
